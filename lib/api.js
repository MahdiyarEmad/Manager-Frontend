...
class APIClient {
  ...
  // Devices
  async getDevices(params = {}, itemsOnly = true) {
    const query = new URLSearchParams(params).toString();
    const data = await this.request(`/devices${query ? `?${query}` : ''}`);
    return itemsOnly ? data?.items || data || [] : data;
  }
  ...
  // Bulk create devices - accepts an array of device objects
  async createDevicesBulk(devices) {
  ...
  async getDeviceLogs(deviceId, params = {}) {
    const query = new URLSearchParams(params).toString();
    return this.request(`/device-logs/${deviceId}/logs${query ? `?${query}` : ''}`);
  }
  ...
  // Tests
  async getTests(params = {}, itemsOnly = true) {
    const query = new URLSearchParams(params).toString();
    const data = await this.request(`/tests${query ? `?${query}` : ''}`);
    return itemsOnly ? data?.items || data || [] : data;
  }
